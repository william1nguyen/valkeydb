...
func buildInfo(connContext *ConnContext, section string) string {
	...
	if show("clients") {
		connected := strconv.FormatInt(connContext.metrics.openConnections.Load(), 10)
		total := strconv.FormatUint(connContext.metrics.totalConnections.Load(), 10)
		lines = append(lines, "connected_clients:"+connected)
		lines = append(lines, "total_connections_received:"+total)
	}
	...
	if show("stats") {
		total := strconv.FormatUint(connContext.metrics.totalCommands.Load(), 10)
		lines = append(lines, "total_commands_processed:"+total)
	}
